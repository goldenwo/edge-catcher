...
def archive_old_analysis_results(
    conn,
    archive_dir: Path,
    days_to_keep: int = 90,
) -> dict:
    """Archive analysis_results older than `days_to_keep` to gzip CSV."""
    archive_dir = Path(archive_dir)
    archive_dir.mkdir(parents=True, exist_ok=True)
    ...
    cutoff = (datetime.now(timezone.utc) - timedelta(days=days_to_keep)).isoformat()
    cursor = conn.execute(
        "SELECT * FROM analysis_results WHERE run_timestamp < ? ORDER BY run_timestamp ASC",
        (cutoff,),
    )
    rows = cursor.fetchall()

    if not rows:
        logger.info("No analysis results older than %d days to archive", days_to_keep)
        return {"rows_archived": 0, "rows_deleted": 0, "archive_file": ""}

    fieldnames = [desc[0] for desc in cursor.description]
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    archive_path = archive_dir / f"analysis_results_{timestamp}.csv.gz"
    tmp_path = archive_path.with_suffix(".csv.gz.tmp")
    ...
    try:
        ...
        try:
            for i in range(0, len(run_ids), batch_size):
                ...
                placeholders = ",".join("?" * len(batch))
                result = conn.execute(
                    f"DELETE FROM analysis_results WHERE run_id IN ({placeholders})", batch
                )
                rows_deleted += result.rowcount
                # Also clean hypothesis_runs if table exists
                try:
                    conn.execute(
                        f"DELETE FROM hypothesis_runs WHERE run_id IN ({placeholders})", batch
                    )
                except Exception:
                    pass
            conn.commit()
    ...
    logger.info("Archived %d analysis results to %s", len(rows), archive_path)
    return {
        "rows_archived": len(rows),
        "rows_deleted": rows_deleted,
        "archive_file": str(archive_path),
...
def archive_old_backtest_results(
    conn,
    archive_dir: Path,
    days_to_keep: int = 90,
) -> dict:
    """Archive backtest_results older than `days_to_keep` to gzip CSV."""
    archive_dir = Path(archive_dir)
    archive_dir.mkdir(parents=True, exist_ok=True)
    ...
    cursor = conn.execute(
        "SELECT * FROM backtest_results WHERE run_timestamp < ? ORDER BY run_timestamp ASC",
        (cutoff,),
    )
    rows = cursor.fetchall()

    if not rows:
        logger.info("No backtest results older than %d days to archive", days_to_keep)
        return {"rows_archived": 0, "rows_deleted": 0, "archive_file": ""}

    fieldnames = [desc[0] for desc in cursor.description]
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    archive_path = archive_dir / f"backtest_results_{timestamp}.csv.gz"
    tmp_path = archive_path.with_suffix(".csv.gz.tmp")
    ...
    try:
        ...
        try:
            for i in range(0, len(task_ids), batch_size):
                ...
                placeholders = ",".join("?" * len(batch))
                result = conn.execute(
                    f"DELETE FROM backtest_results WHERE task_id IN ({placeholders})", batch
                )
                rows_deleted += result.rowcount
            conn.commit()
        except Exception:
    ...
    logger.info("Archived %d backtest results to %s", len(rows), archive_path)
    return {
        "rows_archived": len(rows),
        "rows_deleted": rows_deleted,
        "archive_file": str(archive_path),
