    ...
    def run_hypothesis(self, h: Hypothesis) -> HypothesisResult:
        ...
        if not self.force:
            ...
            if existing_id:
                ...
                if existing:
                    # Re-run promotes that lack validation (pre-validation-pipeline results)
                    if existing.get("verdict") == "promote" and not existing.get("validation_details"):
                        logger.info("Re-running %s/%s — promote lacks validation", h.strategy, h.series)
                    else:
        ...
        # Build preliminary HypothesisResult from backtester JSON
        preliminary = HypothesisResult(
            hypothesis=h,
            status="ok",
        ...
        result = replace(preliminary, verdict=verdict, verdict_reason=verdict_reason)
