...
def _assert_mode_coherence(config: dict) -> None:
	...
	# the paper path is byte-unchanged. Return now for paper — AFTER
	# validating the paper fill-model config. Pure config read; consistent
	# with this gate's no-network/no-DB posture.
	if mode == "paper":
		fill_model = config.get("paper_fill_model", "optimistic")
		if fill_model != "optimistic":
			raise _coherence_fail(
				"paper_fill_model",
				f"must be 'optimistic'; got {fill_model!r}. The 'fixed' slippage model "
				f"was retired 2026-06-23; honest fills are the replay-only fill_latency_ms "
				f"(see replay_capture). A `fill_latency_ms` key in a shared bundle YAML is "
				f"ignored by the live boot path.",
			)
		return
...
def _build_paper_executor(config: dict, market_state: MarketState) -> Executor:
	"""Paper executor. Post-PR#57 the only paper fill model is optimistic; honest
	fills live in the REPLAY path (fill_latency_ms)."""
	return PaperExecutor(market_state=market_state, config=config)
