...
4. **Paper trader.** P&L source of truth. The live paper trader
   ...
   The paper executor uses an **optimistic** fill model: it walks the
   visible orderbook and assumes quoted liquidity fills at the posted
   price. For execution-fidelity evaluation, the **replay backtester**
   supports a `fill_latency_ms` latency model that replays each order
   with arrival-time delay and checks whether the book was still
   crossable — this is the honest fill verdict, and it lives in the
   replay path, not the live boot path.
