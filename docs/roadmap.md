...
## v1.6.0 — Released 2026-05-29 (live-execution cycle, sub-projects A–F)

After v1.3.1 the project's actual direction was **live order execution** — not the UI-parity / desktop-app cycles sketched below (v1.4–v1.6+ candidates), which were written before this pivot and are **deferred** (still valid future directions). Released 2026-05-29 as `v1.6.0` — see [CHANGELOG.md](../CHANGELOG.md) `[1.6.0]`:
...
1. **Reports page** — UI surface for `python -m edge_catcher.reporting`. Pick a date, pick a DB, click Generate; see the rich-formatted body inline plus the raw JSON expandable. New API endpoint `GET /api/reporting/run?db=...&date=...` returning the full `generate_report` dict. ~1 session.
   - Deferrals locked at design time: auto-refresh / live updates → owned by #4; Discord/Slack body preview → owned by #3 ("preview before sending"); period / multi-day views → see "Reports — period views" under v1.6+ candidates; custom DB path input, sortable table headers, CSV/PDF export → v1.4.x patches if requested.
...
3. **"Send to channel" button on Reports page** — reuses the notifications-config endpoints; one click sends the currently-displayed report to a chosen channel. Trivial once #1 + #2 land. Includes a "preview before sending" affordance — only place a Discord-flavored body preview earns its weight (Reports itself stays native-rendered per #1's design).
4. **Paper-trader live monitor** — read-only view of the running paper trader's state (active strategies, open positions, recent fills, today's P&L). Requires a state-export endpoint on the paper trader (or shared SQLite read against `paper_trades.db`). ~1-2 sessions; the harder design question is "what do non-Pi users see if no paper trader is running locally?" — likely a "Not running" empty state + a link to docs. Owns auto-refresh / SSE / live-update concerns for the UI cycle — Reports (#1) stays manual-Generate by design; live monitoring belongs here.
...
- **Unified-layer async + middleware → migrate `engine/notifications.py`** — adds `AsyncChannel` protocol, rate-limiting + bounded-concurrency middleware to `edge_catcher.notifications`. Enables migrating the paper-trader-internal Discord client (`edge_catcher/engine/notifications.py`, originally deprecated in v1.1 at the old `monitors/` path and relocated to `engine/` as part of the v1.5 paper-migration) onto the unified config story. Public users get one notification surface (YAML-config + multi-adapter for the reporting CLI AND the paper trader) instead of two. Was tagged as v1.2 candidate but has been pushed back as v1.4 (UI parity) and v1.5 (Desktop-app UX) take priority.
- **Reports — period views** — range queries (last-7-days, last-30-days, custom range) backed by a new `generate_period_report(start, end)` in `edge_catcher/reporting/`. Different reporting function from v1.4's single-day `generate_report`, not just a UI rearrangement; deserves its own design pass. UI shape: likely a new tab on Reports or a separate `/reports/range` route.
- **Frontend test framework** — vitest + testing-library + baseline tests for all existing pages (Dashboard, Backtest, Research, Settings, etc.). Cross-cutting tooling task — refits all pages, lands its own CI gate. Disproportionate to attach to any single feature PR; deserves its own dedicated task.
