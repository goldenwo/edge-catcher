...
- **Paper trader** — real-time WebSocket dispatch, orderbook-aware sizing, fresh-book gates, multi-strategy concurrency; replay-path honest fill model (`fill_latency_ms`) applies latency-aware arrival-time replay so paper P&L is grounded in actual fill reachability
