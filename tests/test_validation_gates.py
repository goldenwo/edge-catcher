...
class TestReplaceParam:
	...
	def test_replaces_int_default(self):
		...
		out = _replace_param(self.SAMPLE, "Strat41", "threshold_high", 72, "strategy_a__sens_72")
	...
	def test_replaces_float_default(self):
		...
		out = _replace_param(code, "Strat43", "flow_threshold", 0.575, "strategy_c__sens_0_575")
		params = self._reparse_params(out)
	...
	def test_replaces_class_and_name_even_when_values_nonmatching(self):
		...
		out = _replace_param(self.SAMPLE, "Strat41", "threshold_high", 60, "strategy_a__sens_unchanged")
		...
		assert "class strat_34__sens_unchanged(Strategy)" in out
...
class TestTailRiskGate:
	...
	The Apr 11 run had strat-21/KXETH (88% WR, avg_loss/avg_win=6.6x)
	...
	def test_selling_vol_pattern_fails(self):
		...
		# Reproduce strat-21/KXETH-ish shape: mean ≈ 1.1
		pnl = [12] * 88 + [-82] * 12
