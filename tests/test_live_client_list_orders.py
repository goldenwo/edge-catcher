...
async def test_list_orders_happy_path_parses_all(cfg, audit, signing_env, tmp_path):
	...
	body = {
		"orders": [
			_order_json(
				order_id="ord-1", client_order_id="strat-34-KXSOL15M-1",
				status="resting", count=10, filled_count=0,
			),
			_order_json(
				order_id="ord-2", client_order_id="strat-34-KXSOL15M-2",
	...
	assert [o.client_order_id for o in orders] == [
		"strat-34-KXSOL15M-1", "strat-34-KXSOL15M-2",
	]
...
async def test_list_orders_skips_malformed_element_logs_and_continues(
	...
	body = {
		"orders": [
			_order_json(order_id="ord-1", client_order_id="strat-34-KXSOL15M-1"),
			"not-a-dict",  # malformed: _parse_order would AttributeError on .get
			_order_json(order_id="ord-2", client_order_id="strat-34-KXSOL15M-2"),
		],
		"cursor": "",
	}
