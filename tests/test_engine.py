...
class TestProcessTick:
	...
	async def test_enter_signal_records_trade(self, store, config):
		...
		await process_tick(ctx, strategies, store, config, _make_executor_for_ctx(ctx, config), now=_now())
	...
	@pytest.mark.asyncio
	async def test_enter_signal_on_empty_book_is_skipped(self, store, config):
	...
	@pytest.mark.asyncio
	async def test_exit_signal_closes_trade(self, store, config):
	...
	@pytest.mark.asyncio
	async def test_strategy_exception_does_not_crash(self, store, config):
	...
	@pytest.mark.asyncio
	async def test_rejects_100c_no_entry(self, store, config):
	...
	@pytest.mark.asyncio
	async def test_rejects_0c_yes_entry(self, store, config):
...
class TestHandleTickerMsg:
	...
	@pytest.mark.asyncio
	async def test_routes_tick_to_matching_strategy(self, setup):
	...
	@pytest.mark.asyncio
	async def test_ignores_unmatched_series(self, setup):
		ms, store, strategies, strat_by_series, pending_states, config = setup
	...
	@pytest.mark.asyncio
	async def test_rejects_price_outside_range(self, setup):
		ms, store, strategies, strat_by_series, pending_states, config = setup
	...
	@pytest.mark.asyncio
	async def test_ignores_missing_yes_ask(self, setup):
		ms, store, strategies, strat_by_series, pending_states, config = setup
	...
	@pytest.mark.asyncio
	async def test_yes_bid_read_separately(self, setup):
		...
		await _handle_ticker_msg(
			msg, config, ms, store, [no_strat], strat_by_series, pending_states, set(),
			PaperExecutor(market_state=ms, config=config), now=_now(),
		)

	@pytest.mark.asyncio
	async def test_derives_event_ticker(self, setup):
		...
		await _handle_ticker_msg(
			msg, config, ms, store, [strat], strat_by_series, pending_states, set(),
			PaperExecutor(market_state=ms, config=config), now=_now(),
		)

	@pytest.mark.asyncio
	async def test_second_tick_not_first_observation(self, setup):
		ms, store, strategies, strat_by_series, pending_states, config = setup
		msg = self._make_msg("KXBTC15M-26APR10-T100", yes_ask=50)
...
class TestHandleTradeMsg:
	...
	@pytest.mark.asyncio
	async def test_routes_trade_to_strategy(self, setup):
	...
	@pytest.mark.asyncio
	async def test_trade_populates_taker_side_and_count(self, setup):
	...
	@pytest.mark.asyncio
	async def test_trade_updates_price_history(self, setup):
	...
	@pytest.mark.asyncio
	async def test_ignores_unregistered_ticker(self, setup):
	...
	@pytest.mark.asyncio
	async def test_ignores_missing_yes_price(self, setup):
	...
	@pytest.mark.asyncio
	async def test_ignores_price_outside_range(self, setup):
	...
	@pytest.mark.asyncio
	async def test_handles_string_price_and_count(self, setup):
	...
	async def test_bid_ask_sourced_from_orderbook_not_trade(self, setup):
		...
		await _handle_trade_msg(
			msg, config, ms, store, [check_strat], strat_by_series, pending_states, set(),
			PaperExecutor(market_state=ms, config=config), now=_now(),
		)
...
class TestDirtyTracking:
	...
	@pytest.mark.asyncio
	async def test_ticker_msg_marks_dirty(self, setup):
		ms, store, strategies, strat_by_series, pending_states, config = setup
		dirty: set[str] = set()
		msg = {"type": "ticker", "msg": {
		...
		await _handle_ticker_msg(
			msg, config, ms, store, strategies, strat_by_series, pending_states, dirty,
			PaperExecutor(market_state=ms, config=config), now=_now(),
		)
		assert "stub" in dirty

	@pytest.mark.asyncio
	async def test_trade_msg_marks_dirty(self, setup):
		ms, store, strategies, strat_by_series, pending_states, config = setup
		dirty: set[str] = set()
		msg = {"type": "trade", "msg": {
			"market_ticker": "KXBTC15M-26APR10-T100",
...
class TestProcessTickMetrics:
	...
	@pytest.mark.asyncio
	async def test_entry_increments_attempted_and_filled(self, store, config):
	...
	@pytest.mark.asyncio
	async def test_stale_book_skip_increments_counter(self, store, config):
