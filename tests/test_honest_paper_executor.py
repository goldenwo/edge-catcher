...
WHY this exists: P1's strat-34 verdict found Gap-1 — paper win-rate 53.1% vs
...
def _req(*, side: str = "yes", action: str = "buy", strategy: str = "strat_34") -> OrderRequest:
	return OrderRequest(
		...
		strategy=strategy,
		client_order_id="strat_34-KXSOL15M-1715000000000",
		action=action,
	)
...
def test_buy_fill_price_increases_and_metrics_worsen():
	m = _model(per_strategy={"strat_34": 5})
...
def test_per_strategy_override_and_default_fallback():
	m = _model(default=2, per_strategy={"strat_34": 5})
	out_known = m.adjust(_result(blended=42), _req(strategy="strat_34", action="buy"), _OB)
...
def test_determinism_same_input_same_output():
	m = _model(default=3, per_strategy={"strat_34": 5})
