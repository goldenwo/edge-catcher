...
@pytest.mark.asyncio
async def test_filled_routes_record_trade_with_exact_kwargs(now):
	...
	executor = MagicMock()
	# AsyncMock so `await executor.place(...)` returns the OrderResult
	# instead of a coroutine wrapping a MagicMock.
	...
	signal = Signal(
		...
		series="KXSOL15M", strategy="strat-34", reason="signal",
	)
	ctx = MagicMock(yes_ask=42, no_ask=58, orderbook=MagicMock(depth=5))
	config = {"_metrics": MagicMock()}

	await _handle_enter(signal, ctx, store, config, executor, now=now)
	...
	assert kwargs["strategy"] == "strat-34"
...
@pytest.mark.asyncio
async def test_rejected_stale_book_routes_metric_inc_skipped_stale(now):
	store = MagicMock()
	executor = MagicMock()
	executor.place = AsyncMock(return_value=OrderResult(
		status="rejected",
...
@pytest.mark.asyncio
async def test_rejected_other_routes_metric_inc_skipped_other(now):
	store = MagicMock()
	executor = MagicMock()
	executor.place = AsyncMock(return_value=OrderResult(
		status="rejected",
...
@pytest.mark.asyncio
async def test_entry_price_out_of_range_returns_without_calling_executor(now):
